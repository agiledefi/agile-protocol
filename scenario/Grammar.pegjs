  ...
  function getArgValues(eventArgs, macroArgs) {
    ...
    eventArgs.forEach((eventArg) => {
      if (eventArg.argName) {
        ...
        if (usedNamedArg) {
          throw new Error(`Caglot use positional arg after named arg in macro invokation ${JSON.stringify(eventArgs)} looking at ${eventArg.toString()}`);
        }
    ...
    macroArgs.forEach(({arg, def, splat}, argIndex) => {
      if (usedSplat) {
        throw new Error("Caglot have arg after splat arg");
      }
      ...
      if (eventArgNameMap[arg] !== undefined) {
        ...
        throw new Error("Macro caglot find arg value for " + arg);
      }
      argValues[arg] = val;
    });
...
expr
  = (
    ...
    / begin_agile inner:expr end_agile { return [inner]; }
