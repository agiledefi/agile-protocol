...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isAgileStr = await world.web3.eth.getStorageAt(addr.val, isCompKey);

          return new ListV([
            new BoolA(isListed),
            new ExpNumberA(collateralFactor.toString(), 1e18),
            new BoolA(areEqual(userInMarket, 1)),
            new BoolA(areEqual(isAgileStr, 1))
          ]);
        default:
          return new NothingA();
      }
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "AGL ...agileArgs" - Returns AGL value
    `,
    'AGL',
...
let contractFetchers = [
  ...
  { contract: "AgileLens", implicit: false },
