...
async function getAgileMarkets(world: World, comptroller: Comptroller): Promise<ListV> {
  let mkts = await comptroller.methods.getAgileMarkets().call();
...
async function checkIsAgile(world: World, comptroller: Comptroller, aToken: AToken): Promise<BoolA> {
  let {0: isListed, 1: _collateralFactorMantissa, 2: isAgile} = await comptroller.methods.markets(aToken._address).call();
...
export function comptrollerFetchers() {
  return [
    ...
    new Fetcher<{comptroller: Comptroller, aToken: AToken}, BoolA>(`
        ...
        * "Comptroller CheckIsAgile <AToken>" - Returns true if market is listed, false otherwise.
          * E.g. "Comptroller CheckIsAgile aZRX"
      `,
      "CheckIsAgile",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, {comptroller, aToken}) => checkIsAgile(world, comptroller, aToken)
    ),
    ...
    new Fetcher<{comptroller: Comptroller}, ListV>(`
      ...
      * "GetAgileMarkets" - Returns an array of the currently enabled Agile markets. To use the auto-gen array getter agileMarkets(uint), use AgileMarkets
      * E.g. "Comptroller GetAgileMarkets"
      `,
      "GetAgileMarkets",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => await getAgileMarkets(world, comptroller)
     ),

    new Fetcher<{comptroller: Comptroller}, NumberA>(`
      #### AgileRate

      * "AgileRate" - Returns the current agl rate.
      * E.g. "Comptroller AgileRate"
      `,
      "AgileRate",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => new NumberA(await comptroller.methods.agileRate().call())
    ),
    ...
    new Fetcher<{comptroller: Comptroller, signature: StringA, callArgs: StringA[]}, NumberA>(`
        ...
        * "CallNum signature:<String> ...callArgs<CoreValue>" - Simple direct call method
          * E.g. "Comptroller CallNum \"agileSpeeds(address)\" (Address Coburn)"
      `,
      "CallNum",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
    ...
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringA}, NumberA>(`
        #### AgileSupplyState(address)

        * "Comptroller AgileBorrowState aZRX "index"
      `,
      "AgileSupplyState",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.agileSupplyState(AToken._address).call();
        return new NumberA(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringA}, NumberA>(`
        #### AgileBorrowState(address)
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.agileBorrowState(AToken._address).call();
        return new NumberA(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressA, key: StringA}, NumberA>(`
        #### AgileAccrued(address)

        * "Comptroller AgileAccrued Coburn
      `,
      "AgileAccrued",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.agileAccrued(account.val).call();
    ...
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressA}, NumberA>(`
        ...
        * "Comptroller AgileSupplierIndex aZRX Coburn
      `,
      "AgileSupplierIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberA(await comptroller.methods.agileSupplierIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressA}, NumberA>(`
        #### AgileBorrowerIndex

        * "Comptroller AgileBorrowerIndex aZRX Coburn
      `,
      "AgileBorrowerIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberA(await comptroller.methods.agileBorrowerIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberA>(`
        #### AgileSpeed

        * "Comptroller AgileSpeed aZRX
      `,
      "AgileSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
