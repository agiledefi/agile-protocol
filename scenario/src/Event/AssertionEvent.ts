...
async function assertApprox(world: World, given: NumberA, expected: NumberA, tolerance: NumberA): Promise<World> {
...
function getLogValue(value: any): Value {
  if (typeof value === 'number' || (typeof value === 'string' && value.match(/^[0-9]+$/))) {
    return new NumberA(Number(value));
  } else if (typeof value === 'string') {
    return new StringA(value);
  } else if (typeof value === 'boolean') {
    return new BoolA(value);
...
export function assertionCommands() {
  return [
    new View<{ given: NumberA, expected: NumberA, tolerance: NumberA }>(`
        #### Approx
        ...
        * "Approx given:<Value> expected:<Value> tolerance:<Value>" - Asserts that given approximately matches expected.
          ...
          * E.g. "Assert Approx (AToken aZRX TotalSupply) (Exactly 55) 1e-18"
          * E.g. "Assert Approx (AToken aZRX Comptroller) (Comptroller Address) 1"
      `,
      "Approx",
      [
        new Arg("given", getNumberA),
        new Arg("expected", getNumberA),
        new Arg("tolerance", getNumberA, { default: new NumberA(0.001) })
      ],
    ...
    new View<{ given: Value }>(`
      ...
      (world, { given }) => assertEqual(world, given, new BoolA(true))
    ),
    ...
    new View<{ given: Value }>(`
      ...
      (world, { given }) => assertEqual(world, given, new BoolA(false))
    ),
    new View<{ event: EventV, message: StringA }>(`
        #### ReadRevert

        * "ReadRevert event:<Event> message:<String>" - Asserts that reading the given value reverts with given message.
          * E.g. "Assert ReadRevert (Comptroller CheckMembership Geoff aBNB) \"revert\""
      `,
      "ReadRevert",
      [
    ...
    new View<{ event: EventV, message: StringA }>(`
        ...
        * "ReadError event:<Event> message:<String>" - Asserts that reading the given value throws given error
          * E.g. "Assert ReadError (Comptroller Bad Address) \"caglot find comptroller\""
      `,
      "ReadError",
      [
        new Arg("event", getEventV),
    ...
    new View<{ error: StringA, info: StringA, detail: StringA }>(`
        #### Failure
      ...
      "Failure",
      [
        new Arg("error", getStringA),
        new Arg("info", getStringA),
        new Arg("detail", getStringA, { default: new StringA("0") }),
      ],
      (world, { error, info, detail }) => assertFailure(world, new Failure(error.val, info.val, detail.val))
    ),

    new View<{ error: StringA, message: StringA }>(`
        #### RevertFailure
    ...
    new View<{ message: StringA }>(`
      ...
      "Revert",
      [
        new Arg("message", getStringA, { default: new StringA("revert") }),
      ],
    ...
    new View<{ name: StringA, params: MapV }>(`
        #### Log
