...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringA, timelock: AddressA, agl: AddressA, guardian: AddressA },
      GovernorData
    >(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> agl:<Address> guardian:<Address>" - Deploys Agile Governor Alpha
        * E.g. "Governor Deploy Alpha GovernorAlpha (Address Timelock) (Address AGL) Guardian"
    `,
      "Alpha",
      [
        new Arg("name", getStringA),
        new Arg("timelock", getAddressA),
        new Arg("agl", getAddressA),
        new Arg("guardian", getAddressA)
      ],
      async (world, { name, timelock, agl, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(
            world,
            from,
            [timelock.val, agl.val, guardian.val]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> agl:<Address> guardian:<Address>" - Deploys Agile Governor Alpha with a mocked voting period
        * E.g. "Governor Deploy AlphaHarness GovernorAlphaHarness (Address Timelock) (Address AGL) Guardian"
    `,
      "AlphaHarness",
      [
        new Arg("name", getStringA),
