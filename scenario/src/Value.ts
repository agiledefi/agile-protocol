...
export class EventV implements Value {
  ...
  compareTo(world: World, given: Value): boolean {
    if (given instanceof EventV) {
      ...
      throw new Error(`Caglot compare ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
    }
  }

  compareOrder(world: World, given: Value): Order {
    throw new Error(`Caglot compare order of ${typeof this} to ${typeof given} (${this.toString()}, ${given.toString()})`);
  }
...
export class NumberA implements Value {
  ...
  toBoolA(): BoolA {
    ...
    throw new Error(`Caglot convert number ${this.val} into bool`)
  }
