...
#  - ABep20Delegator/*.cvl caglot yet be run with the tool
...
spec/certora/AGL/search.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/AGLCertora.sol \
	--settings -b=4,-graphDrawLimit=0,-assumeUnwindCond,-depth=100 \
	--solc_args "'--evm-version istanbul'" \
	--verify \
	 AGLCertora:$@

spec/certora/AGL/transfer.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/AGLCertora.sol \
...
spec/certora/Governor/%.cvl:
	$(CERTORA_RUN) \
	 ...
	 GovernorAlphaCertora:agl=AGLCertora \
	--verify \
...
spec/certora/aDAI/%.cvl:
	...
	--verify \
	 ADaiDelegateCertora:$@ \
	--settings -cache=certora-run-adai
...
spec/certora/Maximillion/%.cvl:
	$(CERTORA_RUN) \
	 ...
	 spec/certora/contracts/ABNBCertora.sol \
	--link \
	 MaximillionCertora:aBnb=ABNBCertora \
	--verify \
	 MaximillionCertora:$@
