...
describe('ABNB', function () {
  ...
  describe('borrowFresh', () => {
    ...
    it("fails if calculating account new total borrow balance overflows", async () => {
      await pretendBorrow(aToken, borrower, 1e-18, 1e-18, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF');
    ...
    it("fails if calculation of new total borrow balance overflows", async () => {
      await send(aToken, 'harnessSetTotalBorrows', ['0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF']);
  ...
  describe('borrow', () => {
    ...
    it("returns error from borrowFresh without emitting any extra logs", async () => {
      expect(await borrow(aToken, borrower, borrowAmount.add(1))).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'BORROW_CASH_NOT_AVAILABLE');
    });
    ...
    [true, false].forEach(async (benefactorPaying) => {
      ...
      describe(label, () => {
        ...
        it("stores new borrow principal and interest index", async () => {
          ...
          expect(await totalBorrows(aToken)).toEqualNumber(beforeProtocolBorrows.sub(repayAmount));
        });
      });
    });
  });
