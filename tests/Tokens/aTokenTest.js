...
describe('AToken', function () {
  ...
  describe('supplyRatePerBlock', () => {
    ...
    it("has a supply rate", async () => {
      ...
      await send(aToken, 'harnessSetReserveFactorFresh', [bnbMantissa(.01)]);
      await send(aToken, 'harnessExchangeRateDetails', [1, 1, 0]);
      await send(aToken, 'harnessSetExchangeRate', [bnbMantissa(1)]);
  ...
  describe('exchangeRateStored', () => {
    ...
    it("calculates with aTokenSupply and total borrows", async () => {
      ...
      await send(aToken, 'harnessExchangeRateDetails', [aTokenSupply, totalBorrows, totalReserves].map(bnbUnsigned));
      const result = await call(aToken, 'exchangeRateStored');
      expect(result).toEqualNumber(bnbMantissa(.1));
    });
    ...
    it("calculates with cash and aTokenSupply", async () => {
      ...
      expect(
        await send(aToken.underlying, 'transfer', [aToken._address, bnbMantissa(500)])
      ).toSucceed();
      await send(aToken, 'harnessExchangeRateDetails', [aTokenSupply, totalBorrows, totalReserves].map(bnbUnsigned));
      const result = await call(aToken, 'exchangeRateStored');
