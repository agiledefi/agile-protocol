...
describe('GovernorAlpha#state/1', () => {
  let agl, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, agl._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(agl, 'transfer', [acct, bnbMantissa(40000000)]);
    await send(agl, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(agl, 'transfer', [accounts[0], bnbMantissa(40000000)]);
    await send(agl, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(agl, 'delegate', [root], { from: accounts[0] });
