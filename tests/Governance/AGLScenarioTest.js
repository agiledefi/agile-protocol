...
describe('AGLScenario', () => {
  let root, accounts;
  let agl;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    agl = await deploy('AGLScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await agl.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await agl.methods.getPriorVotes(root, 1).send();
