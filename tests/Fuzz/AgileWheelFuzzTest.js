...
describe.skip('AgileWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Agile protocol
  ...
  let initialState = globals => {
    return {
      ...
      agileSupplySpeed: new bn(1),
      agileSupplyIndex: new bn(1),
      agileSupplyIndexSnapshots: {},
      agileSupplyIndexUpdatedBlock: globals.blockNumber,

      agileBorrowSpeed: new bn(1),
      agileBorrowIndex: new bn(1),
      agileBorrowIndexSnapshots: {},
      agileSupplyIndexUpdatedBlock: globals.blockNumber,

      agileAccruedWithCrank: {}, // naive method, accruing all accounts every block
      agileAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see agl distributed. just for fuzz testing.
  ...
  let trueUpAgile = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.agileSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.agileBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates agl accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        agileAccruedWithCrank[account] = get(
          state.agileAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(agileSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      agileAccruedWithCrank: agileAccruedWithCrank,
    };
  };

  // real deal agl index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      agileBorrowSpeed,
      agileBorrowIndex,
      agileBorrowIndexSnapshots,
      agileAccruedWithIndex,
      totalBorrows,
      borrowBalances,
      agileBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(agileBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      agileBorrowIndex = agileBorrowIndex.plus(
        agileBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = agileBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      agileBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      agileAccruedWithIndex[account] = get(agileAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(agileBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      agileBorrowIndexUpdatedBlock: globals.blockNumber,
      agileBorrowIndex: agileBorrowIndex,
      agileBorrowIndexSnapshots: {
        ...state.agileBorrowIndexSnapshots,
        [account]: agileBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      agileAccruedWithIndex,
      totalSupply,
      agileSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(agileSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      agileSupplyIndex = agileSupplyIndex.plus(
        agileSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = agileSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some agl
      agileAccruedWithIndex[account] = get(agileAccruedWithIndex[account]).plus(
        balances[account].times(agileSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      agileSupplyIndexUpdatedBlock: globals.blockNumber,
      agileSupplyIndex: agileSupplyIndex,
      agileSupplyIndexSnapshots: {
        ...state.agileSupplyIndexSnapshots,
        [account]: agileSupplyIndex
      },
      agileAccruedWithIndex: agileAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * agl speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.agileSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.agileBorrowSpeed));

    let actual = Object.values(state.agileAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed agl inaccurately`
    );
  };

  // assert agl distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.agileAccruedWithCrank;
    let actual = state.agileAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpAgile(globals, state);
  };
