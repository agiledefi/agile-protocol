...
describe('InterestRateModel', () => {
  ...
  Object.entries(expectedRates).forEach(async ([kind, info]) => {
    ...
    beforeAll(async () => {
      ...
      blocksPerYear = await call(model, 'blocksPerYear');
    });
    ...
    describe(kind, () => {
      ...
      if (kind == 'jump-rate') {
        ...
        it('handles overflowed cash + borrows', async () => {
          await expect(getBorrowRate(model, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 0)).rejects.toRevert("revert SafeMath: addition overflow");
        });

        it('handles failing to get exp of borrows / cash + borrows', async () => {
          await expect(getBorrowRate(model, 0, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 0)).rejects.toRevert("revert SafeMath: multiplication overflow");
        });
        ...
        it('handles overflow utilization rate times slope + base', async () => {
          const badModel = await makeInterestRateModel({ kind, baseRate: -1, multiplier: 5e48, jump: 5e48 });
        ...
        const tests = [
          ...
          describe(`for jump=${jump}, kink=${kink}, base=${base}, slope=${slope}`, () => {
            ...
            beforeAll(async () => {
              ...
              blocksPerYear = await call(jumpModel, 'blocksPerYear');
            });
