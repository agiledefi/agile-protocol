...
async function xaiCalculateSeizeTokens(comptroller, aTokenCollateral, repayAmount) {
  return call(comptroller, 'liquidateXAICalculateSeizeTokens', [aTokenCollateral._address, repayAmount]);
}
...
describe('Comptroller', () => {
  let root, accounts;
  let comptroller, xaicontroller, xai, aTokenCollateral;
  ...
  describe('liquidateXAICalculateAmountSeize', () => {
    it("fails if either asset price is 0", async () => {
      await setOraclePrice(aTokenCollateral, 0);
      expect(
        await xaiCalculateSeizeTokens(comptroller, aTokenCollateral, repayAmount)
      ).toHaveTrollErrorTuple(['PRICE_ERROR', 0]);
    });

    it("fails if the repayAmount causes overflow ", async () => {
      await expect(
        xaiCalculateSeizeTokens(comptroller, aTokenCollateral, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF')
    ...
    it("reverts if it fails to calculate the exchange rate", async () => {
      ...
      await expect(
        send(comptroller, 'liquidateXAICalculateSeizeTokens', [aTokenCollateral._address, repayAmount])
    ...
    [
      ...
      it(`returns the correct value for ${testCase}`, async () => {
        ...
        await setOraclePriceFromMantissa(aTokenCollateral, collateralPrice);
