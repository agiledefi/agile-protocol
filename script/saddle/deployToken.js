...
(async function() {
  ...
  console.log(`Deploying aToken with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('ABep20Immutable', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(bscscanApiKey, contract._address, 'ABep20Immutable', deployArgs, 0);
