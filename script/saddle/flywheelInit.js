...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim agl for at once. default 100 (~2.3M gas)
...
let filterInitialized = async (borrowersByAToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling agileBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let aTokenAddr of Object.keys(borrowersByAToken)) {
		let speed = await call(Comptroller, 'agileSpeeds', [aTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByAToken[aTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'agileBorrowerIndex',[aTokenAddr, borrower])
					}));
...
let filterBorrowers = (apiAccounts, aTokenList) => {
	return apiAccounts.reduce((acc, account) => {
		...
		for (let borrower of validBorrowers) {
			let atokenAddr = borrower.address;
			acc[atokenAddr] = acc[atokenAddr]
				? acc[atokenAddr].concat(account.address)
				: [account.address];
		}
		return acc;
	}, {});
...
let claimAgileBatch = async (borrowersByAToken, opts) => {
	for (let aTokenAddr of Object.keys(borrowersByAToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(Comptroller, 'claimAgile', [
						chunk,
						[aTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimAgileBatch(unInit, opts);
