...
interface ComptrollerLensInterface {
    ...
    function claimAgile(address) external;
    function agileAccrued(address) external view returns (uint);
}
...
contract AgileLens {
    ...
    struct AGLBalanceMetadata {
        uint balance;
        uint votes;
        address delegate;
    }

    function getAGLBalanceMetadata(AGL agl, address account) external view returns (AGLBalanceMetadata memory) {
        return AGLBalanceMetadata({
            balance: agl.balanceOf(account),
            votes: uint256(agl.getCurrentVotes(account)),
            delegate: agl.delegates(account)
        });
    }

    struct AGLBalanceMetadataExt {
        uint balance;
        uint votes;
        address delegate;
        uint allocated;
    ...
    function getAGLBalanceMetadataExt(AGL agl, ComptrollerLensInterface comptroller, address account) external returns (AGLBalanceMetadataExt memory) {
        uint balance = agl.balanceOf(account);
        comptroller.claimAgile(account);
        uint newBalance = agl.balanceOf(account);
        uint accrued = comptroller.agileAccrued(account);
        uint total = add(accrued, newBalance, "sum agl total");
        uint allocated = sub(total, balance, "sub allocated");

        return AGLBalanceMetadataExt({
            balance: balance,
            votes: uint256(agl.getCurrentVotes(account)),
            delegate: agl.delegates(account),
            allocated: allocated
    ...
    function getAgileVotes(AGL agl, address account, uint32[] calldata blockNumbers) external view returns (AgileVotes[] memory) {
        AgileVotes[] memory res = new AgileVotes[](blockNumbers.length);
        for (uint i = 0; i < blockNumbers.length; i++) {
            res[i] = AgileVotes({
                blockNumber: uint256(blockNumbers[i]),
                votes: uint256(agl.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
