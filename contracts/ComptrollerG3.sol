...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterfaceG1, ComptrollerErrorReporter, Exponential {
    ...
    function mintAllowed(address aToken, address minter, uint mintAmount) external onlyProtocolAllowed returns (uint) {
        ...
        distributeSupplierAgile(aToken, minter);
    ...
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateAgileSupplyIndex(aToken);
        distributeSupplierAgile(aToken, redeemer);
    ...
    function borrowAllowed(address aToken, address borrower, uint borrowAmount) external onlyProtocolAllowed returns (uint) {
        ...
        distributeBorrowerAgile(aToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        distributeSupplierAgile(aTokenCollateral, borrower);
        distributeSupplierAgile(aTokenCollateral, liquidator);
    ...
    function transferAllowed(address aToken, address src, address dst, uint transferTokens) external onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateAgileSupplyIndex(aToken);
        distributeSupplierAgile(aToken, src);
        distributeSupplierAgile(aToken, dst);
    ...
    function _supportMarket(AToken aToken) external returns (uint) {
        ...
        // Note that isAgile is not in active use anymore
    ...
    function setAgileSpeedInternal(AToken aToken, uint agileSpeed) internal {
        uint currentAgileSpeed = agileSpeeds[address(aToken)];
        if (currentAgileSpeed != 0) {
            // note that AGL speed could be set to 0 to halt liquidity rewards for a market
        ...
        } else if (agileSpeed != 0) {
            // Add the AGL market
            Market storage market = markets[address(aToken)];
            require(market.isListed == true, "agile market is not listed");

            if (agileSupplyState[address(aToken)].index == 0 && agileSupplyState[address(aToken)].block == 0) {
                agileSupplyState[address(aToken)] = AgileMarketState({
        ...
        if (agileBorrowState[address(aToken)].index == 0 && agileBorrowState[address(aToken)].block == 0) {
                agileBorrowState[address(aToken)] = AgileMarketState({
                    index: agileInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentAgileSpeed != agileSpeed) {
            agileSpeeds[address(aToken)] = agileSpeed;
            emit AgileSpeedUpdated(aToken, agileSpeed);
        }
    }
    ...
    function distributeSupplierAgile(address aToken, address supplier) internal {
        ...
        agileAccrued[supplier] = supplierAccrued;
    ...
    function distributeBorrowerAgile(address aToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            agileAccrued[borrower] = borrowerAccrued;
    ...
    function claimAgile(address[] memory holders, AToken[] memory aTokens, bool borrowers, bool suppliers) public {
        ...
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (borrowers) {
                ...
                updateAgileBorrowIndex(address(aToken), borrowIndex);
                for (j = 0; j < holders.length; j++) {
                    distributeBorrowerAgile(address(aToken), holders[j], borrowIndex);
                    agileAccrued[holders[j]] = grantAGLInternal(holders[j], agileAccrued[holders[j]]);
                }
            }
            if (suppliers) {
                updateAgileSupplyIndex(address(aToken));
                for (j = 0; j < holders.length; j++) {
                    distributeSupplierAgile(address(aToken), holders[j]);
                    agileAccrued[holders[j]] = grantAGLInternal(holders[j], agileAccrued[holders[j]]);
                }
            }
        }
    ...
    /**
     * @notice Transfer AGL to the user
     ...
     * @param amount The amount of AGL to (possibly) transfer
     * @return The amount of AGL which was NOT transferred to the user
     */
    function grantAGLInternal(address user, uint amount) internal returns (uint) {
        AGL agl = AGL(getAGLAddress());
        uint agileRemaining = agl.balanceOf(address(this));
        if (amount > 0 && amount <= agileRemaining) {
            agl.transfer(user, amount);
            return 0;
        }
        return amount;
    }
     ...
     */
    function _setAgileSpeed(AToken aToken, uint agileSpeed) public {
        require(adminOrInitializing(), "only admin can set agile speed");
        setAgileSpeedInternal(aToken, agileSpeed);
    }
