...
interface ComptrollerImplInterface {
    ...
    function agileXAIRate() external view returns (uint);
    function agileAccrued(address account) external view returns(uint);
    function getAssetsIn(address account) external view returns (AToken[] memory);
    function oracle() external view returns (PriceOracle);

    function distributeXAIMinterAgile(address xaiMinter) external;
}

/**
 * @title Agile's XAI Comptroller Contract
 * @author Agile
 */
contract XAIController is XAIControllerStorageG2, XAIControllerErrorReporter, Exponential {
    ...
    function mintXAI(uint mintXAIAmount) external nonReentrant returns (uint) {
        if(address(comptroller) != address(0)) {
            require(mintXAIAmount > 0, "mintXAIAmount caglt be zero");
            ...
            ComptrollerImplInterface(address(comptroller)).distributeXAIMinterAgile(minter);
    ...
    function repayXAI(uint repayXAIAmount) external nonReentrant returns (uint, uint) {
        if(address(comptroller) != address(0)) {
            require(repayXAIAmount > 0, "repayXAIAmount caglt be zero");
            ...
            updateAgileXAIMintIndex();
            ComptrollerImplInterface(address(comptroller)).distributeXAIMinterAgile(payer);
    ...
    function liquidateXAIFresh(address liquidator, address borrower, uint repayAmount, ATokenInterface aTokenCollateral) internal returns (uint, uint) {
        if(address(comptroller) != address(0)) {
            ...
            /* Revert if seize tokens fails (since we caglot be sure of side effects) */
    ...
    /**
     * @notice Initialize the AgileXAIState
     */
    function _initializeAgileXAIState(uint blockNumber) external returns (uint) {
        ...
        if (isAgileXAIInitialized == false) {
            isAgileXAIInitialized = true;
            uint xaiBlockNumber = blockNumber == 0 ? getBlockNumber() : blockNumber;
            agileXAIState = AgileXAIState({
    ...
    /**
     * @notice Accrue AGL to by updating the XAI minter index
     */
    function updateAgileXAIMintIndex() public returns (uint) {
        uint xaiMinterSpeed = ComptrollerImplInterface(address(comptroller)).agileXAIRate();
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(agileXAIState.block));
        if (deltaBlocks > 0 && xaiMinterSpeed > 0) {
            uint xaiAmount = XAI(getXAIAddress()).totalSupply();
            uint agileAccrued = mul_(deltaBlocks, xaiMinterSpeed);
            Double memory ratio = xaiAmount > 0 ? fraction(agileAccrued, xaiAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: agileXAIState.index}), ratio);
            agileXAIState = AgileXAIState({
                index: safe224(index.mantissa, "new index overflows"),
                block: safe32(blockNumber, "block number overflows")
            });
        } else if (deltaBlocks > 0) {
            agileXAIState.block = safe32(blockNumber, "block number overflows");
        }
    ...
    /**
     * @notice Calculate AGL accrued by a XAI minter
     * @param xaiMinter The address of the XAI minter to distribute AGL to
     */
    function calcDistributeXAIMinterAgile(address xaiMinter) public returns(uint, uint, uint, uint) {
        // Check caller is comptroller
        if (msg.sender != address(comptroller)) {
            return (fail(Error.UNAUTHORIZED, FailureInfo.SET_COMPTROLLER_OWNER_CHECK), 0, 0, 0);
        }

        Double memory xaiMintIndex = Double({mantissa: agileXAIState.index});
        Double memory xaiMinterIndex = Double({mantissa: agileXAIMinterIndex[xaiMinter]});
        agileXAIMinterIndex[xaiMinter] = xaiMintIndex.mantissa;

        if (xaiMinterIndex.mantissa == 0 && xaiMintIndex.mantissa > 0) {
            xaiMinterIndex.mantissa = agileInitialIndex;
        }
        ...
        uint xaiMinterAccrued = add_(ComptrollerImplInterface(address(comptroller)).agileAccrued(xaiMinter), xaiMinterDelta);
