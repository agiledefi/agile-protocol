...
interface ComptrollerLensInterface {
    ...
    function distributeXAIMinterAgile(address xaiMinter, bool distributeAll) external;
}
...
contract XAIControllerG1 is XAIControllerStorageG1, XAIControllerErrorReporter, Exponential {
    ...
    function mintXAI(uint mintXAIAmount) external returns (uint) {
        if(address(comptroller) != address(0)) {
            ...
            ComptrollerLensInterface(address(comptroller)).distributeXAIMinterAgile(minter, false);
    ...
    function repayXAI(uint repayXAIAmount) external returns (uint) {
        if(address(comptroller) != address(0)) {
            ...
            updateAgileXAIMintIndex();
            ComptrollerLensInterface(address(comptroller)).distributeXAIMinterAgile(repayer, false);
    ...
    function updateAgileXAIMintIndex() public returns (uint) {
        uint xaiMinterSpeed = ComptrollerLensInterface(address(comptroller)).agileXAIRate();
    ...
    function calcDistributeXAIMinterAgile(address xaiMinter) public returns(uint, uint, uint, uint) {
        ...
        uint xaiMinterAccrued = add_(ComptrollerLensInterface(address(comptroller)).agileAccrued(xaiMinter), xaiMinterDelta);
